...
function reduxStoreReady( reduxStore ) {
	...
	if ( config.isEnabled( 'css-hot-reload' ) ) {
		require( 'lib/css-hot-reload' )(); // GENERIC
	}
	...
	if ( config.isEnabled( 'rubberband-scroll-disable' ) ) { // REMOVE
		require( 'lib/rubberband-scroll-disable' )( document.body );
	}

	if ( config.isEnabled( 'dev/test-helper' ) && document.querySelector( '.environment.is-tests' ) ) { // SHOULD BE GENERIC, maybe
